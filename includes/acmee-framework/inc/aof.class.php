    ...
    class AcmeeFramework {
        ...
        */
        function generateFields($fields_array) {
            ...
            echo '<h2>';
            printf( __( '%s Settings', 'wpav' ), 'WP Admin View' );
            echo '</h2>';
        ...
        public function fieldLoop($config) {
            ...
            //get options data
            $getoption = $this->aofgetOptions( WPAV_OPTIONS_SLUG );

            echo '<div class="loading_form_text">' . __('Loading...', 'wpav') . '</div>';

            do_action('aof_tab_start');

            $fields_array['tabs'] = $config['wpav_fields']['wpav_tabs'];
            $fields_array['fields'] = $config['wpav_fields']['wpav_fields'];
        ...
        function SaveSettings($post) {
			...
			if(isset($post) && isset($post['aof_options_save'])) {
					...
					if($saved) {
						wp_safe_redirect( admin_url( 'admin.php?page=' . WPAV_MENU_SLUG . '&status=updated' ) );
						exit();
					}
					else {
						wp_safe_redirect( admin_url( 'admin.php?page=' . WPAV_MENU_SLUG . '&status=error' ) );
						exit();
					};
                    ...
                    if( isset($post['aof_import_settings']) && !empty($post['aof_import_settings']) ) {
                        ...
                        if(!empty($settings) && is_serialized($settings)) {
                            ...
                            update_option( WPAV_OPTIONS_SLUG, $settings );
                            wp_safe_redirect( admin_url( 'admin.php?page=' . WPAV_MENU_SLUG . '&status=success' ) );
                            exit();
                        }
                    } else {
                      $save_data = array();
        ...
        function aofsaveOptions($save_options) {
            ...
            if($this->config['multi'] === true) {
                update_site_option( WPAV_OPTIONS_SLUG, $data );
                return true;
            }
            else {
                update_option( WPAV_OPTIONS_SLUG, $data );
                return true;
            }
        }
        ...
        function addTypography($fields) {
            ...
            if(isset($fields['show_color']) && $fields['show_color'] !== false) {
                //wpcolor
                $form_field .= __( 'Color', 'wpav' ) . '<label><input class="aof-wpcolor ' . $fields['id'] . '" type="text" name="' . $fields['id'] . '[color]" value="' . $color . '"  /></label>';
            }

            if(isset($fields['show_font_weight']) && $fields['show_font_weight'] !== false) {
                //font weight
                $form_field .= __( 'Font Weight', 'wpav' ) . '<label><select class="aof_font_weight" name="' . $fields['id'] . '[font-weight]">';
            ...
            if(isset($fields['show_font_style']) && $fields['show_font_style'] !== false) {
                //font style
                $form_field .= __( 'Font Style', 'wpav' ) . '<label><select class="aof_font_style" name="' . $fields['id'] . '[font-style]">';
            ...
            if(isset($fields['show_font_size']) && $fields['show_font_size'] !== false) {
                //font size
                $form_field .= __( 'Font Size', 'wpav' ) . '<label><select class="aof_font_size" name="' . $fields['id'] . '[font-size]">';
                for($i = 9; $i <= 65; $i++) {
        ...
        function saveBtn() {
          ...
          ?>
            <div class="save_options">
				<input type="submit" value="<?php _e('Save Changes', 'wpav'); ?>" class="button button-primary button-large" />
				<?php if($adcnt == null) { $adcnt = 0;?>
       ...
       function getDefaultOptions() {
         $fields_array = $this->config['wpav_fields']['wpav_fields'];
        ...
        */
        function aofLoaddefault($reset=false) {
            $default_options = $this->aofgetOptions( WPAV_OPTIONS_SLUG );
            if ( false === $default_options || empty($default_options) || true === $reset ) {
                $default_options = $this->getDefaultOptions();
                if(!empty($default_options)) {
                    if($this->config['multi'] === true) {
                        update_site_option( WPAV_OPTIONS_SLUG, $default_options );
                    }
                    else {
                        update_option( WPAV_OPTIONS_SLUG, $default_options );
                    }
                }
            }
